...
func processRepos(email string) map[int]int {
	...
	repos := fileLinesToSlice(filePath)
...
func fillCommits(email string, path string, commits map[int]int) map[int]int {
	// inst a git repo object from the path
	...
	// get commits history starting from HEAD
...
func getBeginningOfDay(t time.Time) time.Time {
	...
	starOfDay := time.Date(year, month, day, 0, 0, 0, 0, t.Location())
	return starOfDay
}
